...
## Normalizing output units

If you build figures with `tikzfigure.units`, you can keep dimension values
typed in Python and choose a normalized output unit at render time.
...
This converts `TikzDimension` values during rendering, which is handy when you
want consistent exported TikZ code without manually rewriting every dimension.
...
This sends the LaTeX to a remote API for compilation, no local TeX installation
needed.

## Display size in Jupyter

The `figsize` parameter controls how large the figure appears in Jupyter
notebooks. It does not affect the LaTeX output or saved files:
